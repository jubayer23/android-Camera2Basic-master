...
public abstract class CameraVideoFragment extends Fragment {
    ...
    public static final String VIDEO_DIRECTORY_NAME = "retcamv";
    /**
    ...
    private String getVideoFile(){
        ...
        File mediaFile = new File((directory.getPath() + File.separator + "VID_20200129_145036.mp4"));
    ...
    protected void doReplaceTheFrame() {

        MediaMetadataRetriever retriever = new MediaMetadataRetriever();
//use one of overloaded setDataSource() functions to set your data source
        retriever.setDataSource(getActivity(), Uri.fromFile(mCurrentFile));
        String time = retriever.extractMetadata(MediaMetadataRetriever.METADATA_KEY_DURATION);
        String width = retriever.extractMetadata(MediaMetadataRetriever.METADATA_KEY_VIDEO_WIDTH);
        String height = retriever.extractMetadata(MediaMetadataRetriever.METADATA_KEY_VIDEO_HEIGHT);

        Log.d("DEBUG", width + " " +height + " " + time);

        long movieDurationInMillis = Long.parseLong(time );
        long duration = TimeUnit.MILLISECONDS.toSeconds(movieDurationInMillis);

        String[] complexCommand;

        //int frameCount = (int) (timeInsec * 30);
        int frameCount = (int) (duration * 30);
        ...
        complexCommand =
                new String[]{"-y", "-i", mCurrentFile.getPath(), "-i", getImageFile(), "-filter_complex",
                        "[0:v][1:v]overlay=" + 0 + ":" + 0 +
                                ":enable='between" + "(t," + 0 + "," + duration + 1 + ")"
                                + "'[out]", "-map", "[out]", directory.getPath() + "/outputFrame.mp4"};
        ...
        try {
            ...
            ffmpeg.execute(complexCommand, new FFmpegExecuteResponseHandler() {
                ...
                public void onProgress(String message) {
                    ...
                    //pd.show();
                    showProgressDialog("Cropping...", true, false);
                }
